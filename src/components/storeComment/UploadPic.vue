...
<template>
  <div>
    ...
    <div v-if="pictures.length" class="flex flex-wrap">
      <div
        v-for="(imageUrl, index) in pictures"
        :key="index"
        class="relative w-1/3 p-1 overflow-hidden md:w-1/2"
      >
        <div
          @click="deleteImg(index)"
          class="absolute text-lg font-bold leading-7 text-center text-white bg-red-500 rounded-full cursor-pointer w-7 h-7 top-2 right-2"
        >
          X
        </div>
        <img :src="imageUrl" />
