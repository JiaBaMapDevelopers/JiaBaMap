<template>
  <div class="flex gap-2" @mouseleave="resetHoverIndex">
    <div
      ...
      :class="{
        'bg-slate-200':
          starIndex > starsStore.hoverIndex &&
          starIndex > starsStore.selectIndex,
        'bg-amber-500':
          starIndex <= starsStore.hoverIndex ||
          starIndex <= starsStore.selectIndex,
      }"
    ...
    <p>
      {{ starsStore.hint[0] }}
      <label
        v-if="starsStore.hint[1]"
        class="px-2 py-1 text-sm text-white rounded-full bg-amber-500"
        >{{ starsStore.hint[1] }}</label
      >
    </p>
  </div>
</template>
