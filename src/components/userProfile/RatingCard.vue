...
<template>
  <div
    class="box-border w-full h-screen pt-2 mt-2 overflow-y-auto sm:w-72 scrollbar-hide"
  >
    ...
    <div v-if="restaurants">
      ...
      >
        ...
        <!-- 餐廳排名、名稱 -->
        <div
          class="flex flex-col justify-between flex-shrink-0 w-40 sm:text-xl"
        >
          <div>
          ...
          <div class="flex flex-row items-center mt-3 space-x-1 text-center">
            <div
              class="flex items-center px-2 py-1 space-x-1 text-center text-white bg-orange-600 rounded-2xl"
            >
              <p class="text-sm">{{ item.rating }}</p>
              <font-awesome-icon :icon="['fas', 'star']" class="text-xs" />
            </div>
            <p class="text-sm font-light">
              評論: {{ item.userRatingCount }} 則
            </p>
          </div>
          <p class="mt-2 text-sm">
            均消: {{ item.startPrice }} - {{ item.endPrice }}
          </p>
