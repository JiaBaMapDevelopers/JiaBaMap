...
export const useStore = defineStore("store", () => {
  ...
  const fetchPlaceDetail = async () => {
      ...
      openNow.value = resJson.openNow || false;
  ...
  const fetchStorePhoto = async () => {
    const res = await fetch(
      `${import.meta.env.VITE_BACKEND_BASE_URL}/restaurants/photos/${photoIds.value[0]}`
    );
  ...
  const fetchBannerPhoto = async () => {
    const res = await fetch(
      `${import.meta.env.VITE_BACKEND_BASE_URL}/restaurants/photos/${photoIds.value[1]}`
    );
