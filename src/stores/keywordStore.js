    ...
    filteredResult: (state) => {
      ...
      if (state.selectedCost !== "default") {
        filtered = filtered.filter((place) => {
          ...
          if (state.selectedCost === "cost2")
            return price >= 200 && price < 400;
          if (state.selectedCost === "cost3")
            return price >= 400 && price < 600;
          if (state.selectedCost === "cost4")
            return price >= 600 && price < 800;
          if (state.selectedCost === "cost5")
  ...
  actions: {
    ...
    async handleSearch() {
      ...
      const response = await axios.get(
        `${import.meta.env.VITE_BACKEND_BASE_URL}/restaurants/search?keyword=${
          this.keyword
